		...
		success: function(response){
			...
			for(let i = 0;i < response.length;i++){
				...
				if (response[i]["fields"]['imagen']){
					fila += '<td class="text-center fila-table"><a href="#" class="link" onclick="abrir_modal_detalles(\'/perfil_admin/detalles_turismo/'+response[i]['pk']+'/\');"><img style="width:30px;height:20px;" src="/media/'+ response[i]["fields"]['imagen']+'"/></a></td>';
				}else{
					fila += '<td class="text-center fila-table"><a href="#" class="link" onclick="abrir_modal_detalles(\'/perfil_admin/detalles_turismo/'+response[i]['pk']+'/\');"><img style="width:25px;" src="/static/personal/imagen/empty.png"/></a></td>';
				}
