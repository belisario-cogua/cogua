		...
		success: function(response){
			...
			for(let i = 0;i < response.length;i++){
				...
				if (response[i]["fields"]['imagen']){
					fila += '<td class="text-center fila-table"><a href="#" class="link" onclick="abrir_modal_detalles(\'/perfil_admin/detalles_publicacion/'+response[i]['pk']+'/\');"><i class="fas fa-check" style="color: green;"></i></a></td>';
				}else{
					fila += '<td class="text-center fila-table"><a href="#" class="link" onclick="abrir_modal_detalles(\'/perfil_admin/detalles_publicacion/'+response[i]['pk']+'/\');"><i class="fas fa-times" style="color: red;"></i></td>';
				}
	  ...
	  if (result.isConfirmed) {
		...
		$.ajax({
			...
			success: function(response){
				Swal.fire({
				  icon: 'success',
				  title: 'La publicación ha sido eliminado',
